...
class RPRShadingNodeUber(RPRShadingNode):
    ...
    enable_backscattering: BoolProperty(name="Diffuse", description="Enable Diffuse", default=True, update=update_visibility)
    ...
    def export(self, socket, material_exporter):
        ...
        # temp set to metalness mode
        uber_node.set_input(get_rpr_val("RPRX_UBER_MATERIAL_REFLECTION_MODE"), get_rpr_val("RPRX_UBER_MATERIAL_REFLECTION_MODE_METALNESS"))
        ...
        for socket_name, socket_desc in self.node_sockets.items():
            ...
            if rpr_name and eval(eval_string):
                ...
                print(lobe_name, socket_name, val, rpr_name)
                if val is not None and 'Normal' not in socket_name and 'Displacement' not in socket_name:
    ...
    def draw_buttons(self, context, layout):
        ...
        grid.prop(self, 'enable_diffuse', toggle=True)
        grid.prop(self, 'enable_reflection', toggle=True)
        grid.prop(self, 'enable_coating', toggle=True)
        grid.prop(self, 'enable_sss', toggle=True)
