...
struct SimpleRender{
	...
	SimpleRender(fr_int plugin_id){
		fr_int status = FR_SUCCESS;

		fr_int plugins[] = { plugin_id};
	...
	~SimpleRender(){
		...
		fr_int status = FR_SUCCESS;
		CHECK( frObjectDelete(scene));
		CHECK( frObjectDelete(cube));
		CHECK( frObjectDelete(plane));
		CHECK( frObjectDelete(diffuse));
		CHECK( frObjectDelete(light));
		CHECK( frObjectDelete(matsys));

		CHECK( frContextSetAOV(context, FR_AOV_COLOR, NULL));
		CHECK( frObjectDelete(frame_buffer));
		CHECK( frObjectDelete(context));
	}
...
bool test()
{
	...
	int renderer_count = 100;

	SimpleRender renderer(tahoePluginID);

	bool done = false;
	std::thread render_thread([&](){renderer.render(); done = true;});

	std::vector<SimpleRender*> renderers;
	renderers.resize(1);
	...
	for(int i=0;i!=renderer_count;++i){
        ...
        //fr_int tahoePluginID = frRegisterPlugin("Tahoe64.dll");
        //assert(tahoePluginID!=-1);
        auto renderer = new SimpleRender(tahoePluginID);
		//renderer->render();
		delete renderer;
	}
	render_thread.join();
